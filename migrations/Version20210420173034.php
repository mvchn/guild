 ...
 */
final class Version20210420173034 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__attributes AS SELECT id, name, type, required FROM attributes');
        $this->addSql('DROP TABLE attributes');
        $this->addSql('CREATE TABLE attributes (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, product_id INTEGER NOT NULL, name VARCHAR(255) NOT NULL COLLATE BINARY, type VARCHAR(64) NOT NULL COLLATE BINARY, required BOOLEAN NOT NULL, CONSTRAINT FK_319B9E704584665A FOREIGN KEY (product_id) REFERENCES products (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO attributes (id, name, type, required) SELECT id, name, type, required FROM __temp__attributes');
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__attributes AS SELECT id, name, type, required FROM attributes');
        $this->addSql('DROP TABLE attributes');
        $this->addSql('CREATE TABLE attributes (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, name VARCHAR(255) NOT NULL, type VARCHAR(64) NOT NULL, required BOOLEAN NOT NULL)');
        $this->addSql('INSERT INTO attributes (id, name, type, required) SELECT id, name, type, required FROM __temp__attributes');
        $this->addSql('DROP TABLE __temp__attributes');
        $this->addSql('DROP INDEX IDX_2530ADE68D9F6D38');
        $this->addSql('DROP INDEX IDX_2530ADE64584665A');
